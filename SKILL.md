...
Engines it drives, with the CLI conventions each one needs baked in: `gpt_image_2`, `nano_banana_flash` / `_2` / `_pro` (stills); `seedance_2_5`, `veo3_1_lite`, `gemini_omni_flash_1_1`, `kling3_0`, `wan2_7` (motion; Wan takes the shot's `audio` file and lip-syncs to it, 5-15 s). Duration grids, bounded-clip rules, audio flag per model, JSON-vs-prose prompt, and the resume / lockfile / job-recovery guards are all in `fire.py` - read its docstring. Failure handling is `hf_call.py` (shared with the spine): an empty or 5xx response is looked up by prompt since the fire BEFORE any re-fire, then re-fired with backoff; nsfw never re-fires; downloads are verified against Content-Length and a paid job whose file did not land is recorded for recovery; a 5xx on `account status` reads as API unreachable, never as the wrong account.

Output layout is the spine's: `Elements/Stills/<tag>.png`, `Elements/Clips/<prefix>_<tag>_vNN.mp4`. A regen is the next `vN` beside the prior take (`--takes` / `--from-take` for motion, `--reroll` for stills). A shot's `seed_take` / `end_seed_take` picks which still take motion starts from (the Gate-2 pick, not v01). Nothing is overwritten, renamed or moved. A legacy shotlist keeps its plain `<id>.png` / `<id>.mp4` names so old projects resume untouched.
...
38 tests across fire.py and prefire.py: every refusal path above, the three prefire checks (Drive edit permission, CLI account and workspace, model choice), the cost math with a client rate override, the model duration lints, regen-in-place, the legacy shotlist loader, and the per-model command shapes. The stub fails any test that reaches a `generate create`.
