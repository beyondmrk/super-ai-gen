...
product registry - comes from clients.json beside this script and nowhere else.
    ...
    python fire.py --shotlist <shotlist_x.py> --stage motion          # legacy shotlist-module input
...
MOTION_MODELS = {
    ...
    "veo3_1_lite":  {"dur": ("grid", (4, 6, 8)), "mode": None, "resolution": None,
                     ...
                     "rate": {("", "off"): 1.5, ("", "on"): 1.5},
                     "note": "1.0 silent / 1.5 audio measured 2026-09-16; 1.5 both measured 2026-09-22 on another workspace - probe it"},
...
def load_manifest(path):
    """The general JSON manifest (project / stills / shots)."""
...
def load_shotlist(path):
    """A legacy shotlist MODULE, normalised to the same model. Existing projects keep
...
# ---------------------------------------------------------------- prompts
def locks(P):
    """Locks live in project.locks; legacy manifests carry them at project level."""
...
def main(argv=None):
    ...
    src.add_argument("--shotlist", help="legacy shotlist module (.py)")
